...
PINS: list[Pin] = [
    ...
    Pin("analysis_report.txt", "events entering the analysis (no post-fire window)",
        r"(\d+) compiled events; no post-fire window applied", 320,
        quoted_in="paper methods: no window; see events.py"),
    Pin("analysis_report.txt", "events after the runoff-generated filter",
        r"-> (\d+) runoff-generated", 308,
    ...
    Pin("analysis_report.txt", "summer regime concentration R",
        r"concentration R = ([\d.]+)", 0.89, tol=0.005,
    ...
    Pin("analysis_report.txt", "intense-season-first: flows in intense season",
        ...
        77.4, tol=0.05, after="60m: decisive subset with a fire date"),
    ...
    Pin("first_flow_prediction_report.txt", "summer nearest-branch median error",
        r"JJA: P\(first season\) = \d+%;\s+nearest-branch median \|error\| (\d+) d", 22,
    ...
    Pin("first_flow_prediction_report.txt", "winter window coverage",
        ...
        64, quoted_in="paper: winter is window-predictable"),
    ...
    Pin("seasonality_product_evaluation_report.txt",
        ...
        r"A14-AMS\s+15-min\s+\d+\s+[\d.]+ \|\s+\d+\s+([\d.]+)", 73.8, tol=0.05,
